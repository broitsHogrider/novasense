...
public class GlyphPage extends AbstractFont {

    private final int italicSpacing;

    @Getter
    private final float stretching, spacing;

    public GlyphPage(Font font, char[] chars, float stretching, float spacing, boolean antialiasing) {
        ...
        for (char c : chars) {
            ...
            if (posX + glyph.width >= imgWidth) {
                posX = 1;
                posY += (int) (maxHeight + fontMetrics.getDescent() + 2);
            }

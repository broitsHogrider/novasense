...
public class Config implements IMinecraft {
    ...
    public Config(String name) {
        this.name = name;
        this.file = new File(new File(Minecraft.getInstance().gameDir, "\\novacore\\configs"), name + ".json");
    }
    ...
    private void loadStyleSettings(JsonObject stylesObject) {
        for (Map.Entry<String, JsonElement> entry : stylesObject.entrySet()) {
            ...
            if (style != null && styleObject.has("selected")) {
                ...
                if (isSelected) {
                    NovaCore.getInstance().getStyleManager().setCurrentStyle(style);
                }
            }
        }
    }
    ...
    private void loadFunctionSettings(JsonObject functionsObject) {
        NovaCore.getInstance().getFunctionRegistry().getFunctions().forEach(f -> {
    ...
    private void saveFunctionSettings(JsonObject functionsObject) {
        NovaCore.getInstance().getFunctionRegistry().getFunctions().forEach(module -> {
    ...
    private void saveStyleSettings(JsonObject stylesObject) {
        for (Style style : NovaCore.getInstance().getStyleManager().getStyleList()) {
            ...
            styleObject.addProperty("selected", NovaCore.getInstance().getStyleManager().getCurrentStyle() == style);

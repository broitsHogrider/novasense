...
import ru.novacore.utils.render.RenderUtils.Render2D;
import ru.novacore.utils.render.font.Fonts;

import java.util.concurrent.CopyOnWriteArrayList;
import java.util.concurrent.TimeUnit;

@FunctionInfo(name = "Xray Bypass", category = Category.Render)
public class XrayBypass extends Function {
    ...
    public void onDisplay(EventDisplay e) {
        ...
        Render2D.drawRound(mc.getMainWindow().getScaledWidth() / 2f - width / 2, 10, width, heigth, 4, ColorUtils.rgba(10, 10, 10, 128));
        float x = mc.getMainWindow().getScaledWidth() / 2f - width / 2;
        Fonts.interMedium.drawText(e.getMatrixStack(), clicked.size() + "/" + waiting.size(), x + 5, 15, -1, 8);
        ...
        Fonts.interMedium.drawText(e.getMatrixStack(), time, x + width - Fonts.interMedium.getWidth(time, 8) - 5, 15, -1, 8);

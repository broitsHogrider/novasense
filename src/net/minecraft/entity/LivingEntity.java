...
public abstract class LivingEntity extends Entity {
    ...
    protected void jump() {
        ...
        if (this.isSprinting()) {
            ...
            if (this instanceof ClientPlayerEntity) {
                f1 = this.rotationYaw * ((float) Math.PI / 180F);
            }

            if (attackAura.isState() && attackAura.getOptions().get(5).get()) {
                f1 = attackAura.getRotateVector().x * ((float) Math.PI / 180F);
            }
    ...
    public void travel(Vector3d travelVector) {
        if (this.isServerWorld() || this.canPassengerSteer()) {
            ...
            if (this.isInWater() && this.func_241208_cS_() && !this.func_230285_a_(fluidstate.getFluid())) {
                ...
                if (attackAura.isState() && attackAura.getOptions().get(5).get() && this instanceof ClientPlayerEntity) {
                    vector3d1 = this.getVectorForRotation(attackAura.getRotateVector().y, attackAura.getRotateVector().x);
                }
                float f = this.rotationPitch * ((float) Math.PI / 180F);
                if (attackAura.isState() && attackAura.getOptions().get(5).get() && this instanceof ClientPlayerEntity) {
                    f = attackAura.getRotateVector().y * ((float) Math.PI / 180F);
                }

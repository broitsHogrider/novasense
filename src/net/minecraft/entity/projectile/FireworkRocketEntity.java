...
public class FireworkRocketEntity extends ProjectileEntity implements IRendersAsItem
{
    ...
    public void tick() {
        ...
        if (this.isAttachedToEntity()) {
            ...
            ElytraBoost elytraBoost = functionRegistry.getElytraBoost();
            AttackAura attackAura = functionRegistry.getAttackAura();


            if (this.boostedEntity != null) {
                if (this.boostedEntity.isElytraFlying()) {
                    if (attackAura.isState() && attackAura.getOptions().get(5).get() && boostedEntity instanceof ClientPlayerEntity) {
                        Vector3d vector3d = this.getVectorForRotation(attackAura.getRotateVector().y, attackAura.getRotateVector().x);
                        Vector3d vector3d1 = this.boostedEntity.getMotion();
                        if (elytraBoost.state && elytraBoost.modeSetting.is("BravoHVH")) {
                            this.boostedEntity.setMotion(vector3d1.add(vector3d.x * 0.1D + (vector3d.x * 1.69D - vector3d1.x) * 0.5D, vector3d.y * 0.1D + (vector3d.y * 1.69D - vector3d1.y) * 0.5D, vector3d.z * 0.1D + (vector3d.z * 1.69D - vector3d1.z) * 0.5D));
                        } else if (elytraBoost.state && elytraBoost.modeSetting.is("ReallyWorld")) {
                            this.boostedEntity.setMotion(vector3d1.add(vector3d.x * 0.1D + (vector3d.x * 1.65D - vector3d1.x) * 0.5D, vector3d.y * 0.1D + (vector3d.y * 1.65D - vector3d1.y) * 0.5D, vector3d.z * 0.1D + (vector3d.z * 1.65D - vector3d1.z) * 0.5D));
                        } else {
